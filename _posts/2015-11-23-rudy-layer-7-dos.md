...
# Intro to R.UD.Y. attacks

![Mr. Robot]({{ site.baseurl }}images/rudy-post/mr-robot.jpg)

I'm a huge Mr. Robot fan. In the one of the most memorable scenes in the pilot episode, Elliot is called in to work to stop what appears to be a massive DDoS attack against EvilCorp's servers. When it becomes clear that the attack is not a DDoS at all, and that he's dealing with a much more subtle threat coming from a worm within EvilCorp's infrastructure, he exclaims excitedly "Is this a R.U.D.Y. attack? This is awesome!" 

<iframe width="420" height="315" src="https://www.youtube.com/embed/aNJXS9X0yY0" frameborder="0" allowfullscreen></iframe>


It turns out that R.U.D.Y. attacks are actually a real thing, and are quite awesome indeed. They were first written about by Hybrid Security, who named their proof of concept __ru-dead-yet__ after the song by Children of Bodom. R.U.D.Y is an application layer attack. This means that unlike volume or protocol based attacks, the goal of a R.U.D.Y. is to use seemingly legitimate HTTP traffic to crash the target web server.
...
In this post we're going to be building a tool for launching R.U.D.Y. attacks, in direct homage of the original __ru-dead-yet__ script, taking a close look at the core logic that makes them so effective. We're then going to demo the tool to see a R.U.D.Y. attack in action, before going over some effective mitigation techniques that we can employ to protect ourselves from tools such as the one we just built.

# Building RU-DEAD-YET - Initial Setup

Before we begin, we're going to need to install the following dependencies. Let's first create a new project directory called __rudydos__, along with a new virtualenv within it:
...
Next we should create two new files - pip.req and run.py:
...
We should then open up our pip.req file and add the following lines to enumerate dependencies:
...
Finally, we should then use pip to install all of the dependencies we just listed in our pip.req file:
...
Our launch\_attack() function takes three arguments - cid, configs, and headers. The cid (connection id) argument is basically an id number for the process that we set in the calling function. We use it to identify what process we are currently in as we are printing output to the terminal. The second parameter, configs, is a dict containing the target host, target port, sleep time, and possibly proxy information if proxies are currently enabled. The third  parameter contains the specially crafted HTTP headers we made using craft\_headers().
...
The rest of the code in the script is basically dedicated to parsing command line arguments and setting configurations. I'm omitting a detailed explanation about how this portion of the script works for the sake of brevity. With that said, feel free to email if you have any questions about how the script's setup code works.
...
Finally, we use the following command to restart Apache so that our changes take effect.
...
As you can see, it's not a perfect solution. The R.U.D.Y script was able to keep the target server out of commission for well over a minute before being banned by mod-security. That kind of downtime could very well lead to a significant loss of revenue on a production system. It would not be much of stretch to modify our existing script to relaunch connections through fresh SOCKS proxies each time a connection gets banned.

The best defense against these attacks seems to be using a non-blocking server such as NGinx instead of Apache. Check out what happens when we try DOSing the same site with our R.U.D.Y script, but this time running NGinx as the web server instead of Apache.

<iframe width="560" height="315" src="https://www.youtube.com/embed/AfzhRxtNapg" frameborder="0" allowfullscreen></iframe>

Despite launching 500 slow POST connections against the target server, NGinx happily keeps serving the web page. This is because NGinx serves content asynchronously, so incomplete requests are simply moved to the background while NGinx's event loop keeps working on other things.
